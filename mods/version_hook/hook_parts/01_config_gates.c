...
// Cutout prepass probe. ANSWERED 2026-08-20, and the answer closed the whole
// cutout-AA line of work - see FEATURES.md, "Cutout AA: the structural
// verdict". Measured, one run, no overlap:
//
//   ps_130C02F5/2B84B7D3/77D363F6/513B2A00   MULTI_SAMPLE only  (924 draws)
//   ps_8676670C/658CC589                     MS_DEPTH only     (2106 draws)
//
// Foliage IS in the depth prepass, and the prepass carves the silhouette with
// texkill, which kills every MSAA sample of a pixel together. Depth is binary
// per sample by construction, so the silhouette everything downstream inherits
// is hard-edged before the colour pass ever runs. Retired to 0; flip to 1 to
// re-measure if shader identities ever change.
#define ENABLE_CUTOUT_PROBE   0
